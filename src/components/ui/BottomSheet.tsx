...
export interface BottomSheetProps {
  ...
  /** Visible height (px) in the 'peek' snap. Default 132. */
  peekHeight?: number;
 ...
 * `calc()` on `dvh`, which makes SSR output correct with no measurement flash;
 * during a drag we use measured pixels. The scroll area receives bottom padding
 * equal to the hidden portion so the last item can always be scrolled into the
 * visible region.
 */
export function BottomSheet({
  ...
  const snapHeights = useCallback(
    (): Record<SheetSnap, number> => ({
      peek: Math.min(peekHeight, fullHeight || peekHeight),
      half: Math.min(viewportH * 0.5, fullHeight || viewportH * 0.5),
      full: fullHeight,
    }),
    [peekHeight, fullHeight, viewportH],
  );
  ...
  const restingOffset: string = !open
    ...
    : snap === 'full'
      ...
      : snap === 'half'
        ...
        : `calc(100dvh - ${FULL_TOP_GAP + peekHeight}px)`;
  ...
  const scrollStyle: CSSProperties = {
    ...
    paddingBottom: open ? `calc(${offset} + env(safe-area-inset-bottom))` : 0,
  };
      ...
      >
        <div className="flex justify-center pt-2 pb-1" data-handle="true">
          <button
            ...
            className="flex h-8 w-full max-w-[120px] items-center justify-center rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-900"
          >
